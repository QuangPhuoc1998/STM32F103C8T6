...
// global variable
uint32_t freq_value, duty_value, re_duty_value;
uint8_t step_value;
char buffer[5];
char set_pre = 1;
bool status = false;
...
uint8_t m = 0;
int8_t error;
uint16_t set_speed, max_speed, min_speed, re_set_speed;
...
void mode_main()
{
	...
	ILI9341_printText("Control speed",45,75,DARKBLUE,WHITE,3);
	ILI9341_printText("Bluetooth",75,130,DARKBLUE,WHITE,3);
...
void mode_control()
{
  ...
  HAL_TIM_Encoder_Start(&htim2,TIM_CHANNEL_2);
	...
	//
	ILI9341_printText("Set:        rpm",10,50,DARKBLUE,WHITE,2);
	...
	while(1)
	{
		...
		if(myTS_Handle.isPressed)
		{
	      ...
	      ILI9341_fillTriangle(20,3,20,27,0,15,COLOR_WHITE);
			...
			if(myTS_Handle.X > 235 && myTS_Handle.X < 315 && myTS_Handle.Y > 140 && myTS_Handle.Y < 180) 
			{
				...
				if( status == false); else HAL_TIM_Base_Start_IT(&htim3);		
			}
			...
			if(myTS_Handle.X > 0 && myTS_Handle.X < 230 && myTS_Handle.Y > 200 && myTS_Handle.Y < 239) 
			{
				mode_prescaler();
			}
		}
		...
		if(HAL_GPIO_ReadPin(GPIOC, GPIO_PIN_15)==0)
	  {
		...
		set_speed = TIM2 -> CNT;
		if(set_speed != re_set_speed)
		{
			if(set_speed < min_speed )
			{
				htim2.Instance -> CNT = max_speed;
				set_speed = max_speed;
			}
			else if( set_speed > max_speed)
			{
				htim2.Instance -> CNT = min_speed;
				set_speed = min_speed;
			}
			//
			duty_value = (double)(240000000/(set_speed*2*max_step));
			re_set_speed = set_speed;
	  }
		//
		htim3.Instance -> ARR = duty_value;
		// set_speed
		ILI9341_drawChar(70, 50,set_speed/10000+48,DARKBLUE,WHITE,2);
		ILI9341_drawChar(85, 50,set_speed%10000/1000+48,DARKBLUE,WHITE,2);
		ILI9341_drawChar(100, 50,set_speed%1000/100+48,DARKBLUE,WHITE,2);
		ILI9341_drawChar(115, 50,set_speed%100/10+48,DARKBLUE,WHITE,2);
		ILI9341_drawChar(130, 50,set_speed%10+48,DARKBLUE,WHITE,2);
		// freq_value
		freq_value = 2000000/(duty_value);
...
/**-------------Ham giao tiep bluetooth----------------*/
void mode_bluetooth()
{
	htim3.Instance -> ARR = 8000;
  ...
  HAL_GPIO_WritePin(GPIOB, GPIO_PIN_8, GPIO_PIN_RESET);	
	...
	while(1)
	{
		...
		if(bluetooth_status == true)
		{
			...
			ILI9341_drawChar(70, 100,(step/2)/1000+48,DARKBLUE,WHITE,2);
			ILI9341_drawChar(85, 100,(step/2)%1000/100+48,DARKBLUE,WHITE,2);
			ILI9341_drawChar(100, 100,(step/2)%100/10+48,DARKBLUE,WHITE,2);
			ILI9341_drawChar(115, 100,(step/2)%10+48,DARKBLUE,WHITE,2);
			//
